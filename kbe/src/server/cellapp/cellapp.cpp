...
//-------------------------------------------------------------------------------------	
void Cellapp::onShutdown(bool first, bool last)
{
	EntityApp<Entity>::onShutdown(first, last);

	Entities<Entity>::ENTITYS_MAP& entities =  this->pEntities()->getEntities();

	//last为true表示马上shutdown，此时立即销毁所有entity，避免space销毁和space中的ghostEntity销毁之间存在时间差（如果在这个间隙内进程退出进程会崩溃，见KBEN-264）
	if (last)
	{
...
//-------------------------------------------------------------------------------------
void Cellapp::onUpdateDataFromClientOnParent(Network::Channel* pChannel, KBEngine::MemoryStream& s)
{
	ENTITY_ID srcEntityID = 0;
	...
	if (e == NULL)
	{
		WARNING_MSG(fmt::format("Cellapp::onUpdateDataFromClientOnParent: not found entity {}!\n", srcEntityID));
	...
	e->onUpdateDataFromClientOnParent(s);
	s.done();
}
...
//-------------------------------------------------------------------------------------
void Cellapp::onUpdateDataFromClientForControlledEntityOnParent(Network::Channel* pChannel, KBEngine::MemoryStream& s)
{
	ENTITY_ID proxiesEntityID = 0;
	s >> proxiesEntityID;
	if (proxiesEntityID <= 0)
	...
	if (e == NULL)
	{
		ERROR_MSG(fmt::format("Cellapp::onUpdateDataFromClientForControlledEntityOnParent: not found entity {}!\n", srcEntityID));
...
int Cellapp::collideVertical(SPACE_ID spaceID, int layer, uint16 flags, const Position3D& position, const float startDeviationY, const float endDeviationY, std::vector<Position3D>& hitPos)
{
	...
	if (pSpace->pNavHandle() == NULL)
	{
		ERROR_MSG(fmt::format("Cellapp::collideVertical: space({}) not addSpaceGeometryMapping! layer={}\n",
			spaceID, layer));
...
PyObject* Cellapp::__py_collideVertical(PyObject* self, PyObject* args)
{
	uint16 currargsSize = (uint16)PyTuple_Size(args);
	...
	PyObject* pyPos = NULL;
	float startDeviationY = 0.0f;
	float endDeviationY = 0.0f;

	if (currargsSize == 4)
	{
		if (PyArg_ParseTuple(args, "IOff", &spaceID, &pyPos, &startDeviationY, &endDeviationY) == -1)
		{
			PyErr_Format(PyExc_TypeError, "Cellapp::collideVertical: args is error!");
			PyErr_PrintEx(0);
			return 0;
		}
	}
	...
	{
		if (PyArg_ParseTuple(args, "IiOff", &spaceID, &layer, &pyPos, &startDeviationY, &endDeviationY) == -1)
		{
			PyErr_Format(PyExc_TypeError, "Cellapp::collideVertical: args is error!");
			PyErr_PrintEx(0);
			return 0;
	...
	{
		if (PyArg_ParseTuple(args, "IiHOff", &spaceID, &layer, &flags, &pyPos, &startDeviationY, &endDeviationY) == -1)
		{
			PyErr_Format(PyExc_TypeError, "Cellapp::collideVertical: args is error!");
			PyErr_PrintEx(0);
			return 0;
	...
	{
		PyErr_Format(PyExc_TypeError, "Cellapp::collideVertical: args1(pyPos) not is PySequence!");
		PyErr_PrintEx(0);
		return 0;
	}
	...
	{
		PyErr_Format(PyExc_TypeError, "Cellapp::collideVertical: args1(pyPos) invalid!");
		PyErr_PrintEx(0);
		return 0;
	}
